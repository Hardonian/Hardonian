...
Hardonia operates a local-first AI systems lab and product platform. Runtime status is intentionally read from the current operator evidence rather than hard-coded into a public README.
...
Current local truth is published through the authenticated Command Center endpoint `/api/truth` and the read-only artifact `state/truth-latest.json` in the lab. The envelope separates technical health, evidence freshness, commercial readiness, and provider-correlated payment evidence.
...
- [Connect with Scott Hardie on LinkedIn](https://www.linkedin.com/in/scottrmhardie/)
- [Visit the AI Automated Systems website](https://www.aiautomatedsystems.ca)
- [View Scott Hardie's GitHub profile](https://github.com/Hardonian)
- [Email Scott Hardie](mailto:scottrmhardie@gmail.com)
