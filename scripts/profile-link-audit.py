...
def audit():
    ...
    for raw in urls:
        ...
        if raw.startswith('https://') or raw.startswith('http://'):
            ...
            if raw.startswith('/Hardonian/'):
                target='https://github.com'+raw
        targets_to_check.append((target, raw))
    ...
    def check(item):
        ...
        try:
            ...
            with urllib.request.urlopen(req,timeout=20) as r:
                code=r.status
                if code >= 400 and code not in (403,429,530,999):
                    return (raw,code,r.headers.get('content-type','')), None
                return None, f'OK {code} {raw}'
        except urllib.error.HTTPError as e:
            if e.code in (403,429,530,999):
                return None, f'WARN {e.code} {raw}'
            else:
                return (raw,e.code,str(e)), f'FAIL {e.code} {raw}'
        except Exception as e:
            return (raw,'ERROR',str(e)), f'FAIL ERROR {raw}: {e}'

    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for f, msg in executor.map(check, targets_to_check):
            if f:
                fail.append(f)
            if msg:
                print(msg)
